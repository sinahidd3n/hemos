...
namespace hemo {
WBCModel::WBCModel(Config & modelCfg_, HemoCellField & cellField_) : CellMechanics(cellField_, modelCfg_), 
                  cellField(cellField_),
                  k_volume( WBCModel::calculate_kVolume(modelCfg_,*cellField_.meshmetric) ),
                  k_area( WBCModel::calculate_kArea(modelCfg_,*cellField_.meshmetric) ), 
                  k_link( WBCModel::calculate_kLink(modelCfg_,*cellField_.meshmetric) ), 
                  k_bend( WBCModel::calculate_kBend(modelCfg_,*cellField_.meshmetric) ),
                  eta_m( WBCModel::calculate_etaM(modelCfg_))
  { };

void WBCModel::ParticleMechanics(map<int,vector<HemoCellParticle *>> & particles_per_cell, const map<int,bool> & lpc, pluint ctype) {
...
void WBCModel::solidifyMechanics(const std::map<int,std::vector<int>>& ppc,std::vector<HemoCellParticle>& particles,plb::BlockLattice3D<T,DESCRIPTOR> * fluid,plb::BlockLattice3D<T,CEPAC_DESCRIPTOR> * CEPAC, pluint ctype) {
  hemo::Array<T,3> * pos;
